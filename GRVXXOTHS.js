...
const jsonFilePath = filepath.jsoninputpath3;
                  ...
                  "reservationPaymentMethods": [
                    {
                      "paymentCard": {
                        "cardType": "Ax",
                        "cardNumber": 7800393421530109000,
                        "cardNumberMasked": "XXXXXXXXXXXX9000",
...
it("PUT Copy Reservation", async function ({ supertest }) {
  await supertest
      .request(reservation.request)
      .post(`/rsv/v1/hotels/GRVZZ/reservations/${reservationId}/copy`)
      .set('Content-Type', reservation['Content-Type1'])
      .set('x-hotelid',  reservation.hotelId)
      .set('x-app-key', reservation['x-app-key'])
      .set('bypass-routing', reservation['bypass-routing'])
        ...
        await supertest
            ...
            .then(function (response) {
              console.log(response.text)
                ...
                console.log("External Confirmation Number:", externalConfirmationNumber);
                console.log("PMS Confirmation Number:", pmsConfirmationNumber);
              //  // Assuming responseData is the JSON response object


// Log the paym

  //   if (reservation.payment && reservation.payment.paymentInformation && reservation.payment.paymentInformation.paymentCard) {
  //       paymentToken = reservation.payment.paymentInformation.paymentCard.cardNumber.token || 'Not found';
  //       tokenProviderName = reservation.payment.paymentInformation.paymentCard.tokenProviderName || 'Not found';
  //       console.log("Payment Token:", paymentToken);
  //   console.log("Token Provider Name:", tokenProviderName);
  //   }
//   if (
//     reservation.payment &&
//     reservation.payment.paymentInformation &&
//     reservation.payment.paymentInformation.paymentCard &&
//     reservation.payment.paymentInformation.paymentCard.cardNumber
// ) {
//     const paymentCard = reservation.payment.paymentInformation.paymentCard;
//     const paymentToken = paymentCard.cardNumber.token || 'Not found';
//     const tokenProviderName = paymentCard.tokenProviderName || 'Not found';

//     console.log("Payment Token:", paymentToken);
//     console.log("Token Provider Name:", tokenProviderName);
// } else {
//     console.log("Payment information not available.");
// }

let paymentToken = '';
let tokenProviderName = '';

if (reservation.payment && reservation.payment.paymentInformation) {
    const paymentCard = reservation.payment.paymentInformation.paymentCard;

    if (paymentCard && paymentCard.cardNumber && paymentCard.cardNumber.token) {
        paymentToken = paymentCard.cardNumber.token;
    }

    if (paymentCard && paymentCard.tokenProviderName) {
        tokenProviderName = paymentCard.tokenProviderName;
    }
}
